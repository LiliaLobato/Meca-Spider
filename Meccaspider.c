...
void Meccaspider_left_motor_one_step(void)
{
	...
	{
		if(TRUE == get_pbn_flag(LMC_SW2)) //waiting for the motors to complete its period.
		{
		...
		if(TRUE == get_pbn_flag(LMC_SW1))//half of the period?
		{
...
void Meccaspider_right_motor_one_step(void)
{
	...
	while(FALSE == out_of_while)
	{
		if(TRUE == get_pbn_flag(RMC_SW2)) //waiting for the motors to complete its period.
		{
			PORTC->PCR[bit_2] &= ~PORT_PCR_MUX(0x4); //period completed, turn off motors.
			clear_pbn_flag(RMC_SW2);
			out_of_while = TRUE;
		}
		if(TRUE == get_pbn_flag(RMC_SW1))//half of the period
		{
